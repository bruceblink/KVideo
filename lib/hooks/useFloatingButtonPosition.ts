...
export function useFloatingButtonPosition({
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      const nextPosition = reanchorOnResize
        ? clampPosition(fallbackPosition.x, basePosition.y, width, height)
        : clampPosition(basePosition.x, basePosition.y, width, height);
  ...
  }, [clampPosition, getDefaultPosition, reanchorOnResize, storageKey]);
