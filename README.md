[![CI](https://github.com/bruceblink/KVideo/actions/workflows/ci.yml/badge.svg)](https://github.com/bruceblink/KVideo/actions/workflows/ci.yml)
[![Ask DeepWiki](https://deepwiki.com/badge.svg)](https://deepwiki.com/bruceblink/KVideo)
...
[![React](https://img.shields.io/badge/React-19.2.7-149ECA?style=for-the-badge&logo=react)](https://react.dev/)
[![Tailwind CSS](https://img.shields.io/badge/Tailwind-4.3.3-38B2AC?style=for-the-badge&logo=tailwind-css)](https://tailwindcss.com/)
...
> 一个基于 Next.js 16、React 19 和 Tailwind CSS v4 构建的现代化视频聚合播放客户端，聚焦自托管部署、多源并行搜索、播放器体验、账户隔离和可选的跨设备同步服务。

**当前版本：[`0.1.0`](CHANGELOG.md)**

> [!IMPORTANT]
> KVideo 的付费方向仅包括账户、跨设备同步、备份、恢复、设备管理及相关云端基础设施服务。KVideo 不销售、分发、提供或授权任何视频、IPTV、弹幕或其他内容资源，也不会将视频源作为套餐权益。

> [!NOTE]
> 仓库默认不内置任何视频源或 IPTV 源。所有内容来源均由部署者或用户自行配置，并由配置者负责确认来源已获授权、可合法使用且允许当前部署方式访问。
...
KVideo 的本地搜索、播放和用户自有视频源能力不以云同步套餐为前提。SyncHub 是可选的独立云服务，只负责用户数据同步及后续的备份、恢复和设备管理。

> [!IMPORTANT]
> 当前产品主线是打磨视频播放体验。同步服务商业化将在播放成功率、故障恢复、跨端稳定性和真实用户需求达到约定门槛后再启动。详细顺序见 [播放器优先开发计划](docs/player-first-development-plan.md)。
...
- **按标题去重的历史记录**
- **普通空间与独立空间隔离**
- **兼容入口**：现有独立空间仍通过 `/premium` 访问
- **独立配置**：两种空间分别保存来源配置、推荐、收藏、历史与设置

> [!NOTE]
> 代码中的 `premium` 命名和 `PREMIUM_PASSWORD` 是现有独立数据空间的兼容名称，不表示 VIP、付费视频、高级视频源或平台提供的内容。后续版本将迁移到不含商业含义的命名。
...
- **独立空间兼容密码**：`PREMIUM_PASSWORD`，仅保护现有 `/premium` 数据空间，不代表云同步订阅
...
- **SyncHub 同步**：默认连接 `https://sync.likanug.app`，使用有效的 KVideo API Key 同步观看历史和收藏；也可连接自建 SyncHub，未配置 Key 时继续兼容现有 Redis 同步

### 云同步服务边界

- **付费对象**：SyncHub 账户、跨设备同步、备份恢复、设备管理及相应云端基础设施
- **非付费对象**：视频源、IPTV 源、弹幕源、视频内容、搜索结果和播放权限
- **来源归属**：内容来源始终由部署者或用户自行配置，KVideo 与 SyncHub 不提供来源目录或套餐专属来源
- **本地能力**：未订阅 SyncHub 不影响 KVideo 的本地搜索、播放、收藏、历史和来源配置
- **当前同步范围**：观看历史和收藏；设置、用户自有来源配置、设备管理与版本恢复属于后续规划

同步服务的长期产品边界和候选阶段见 [同步服务开发计划](docs/sync-service-development-plan.md)；当前执行顺序以 [播放器优先开发计划](docs/player-first-development-plan.md) 为准。
...
| `PREMIUM_PASSWORD` | - | 现有 `/premium` 独立数据空间的兼容密码；与 SyncHub 套餐无关 |
...
| 技术 | 当前版本 | 用途 |
|------|----------|------|
| [Next.js](https://nextjs.org/) | `16.2.10` | App Router、服务端组件、构建链路 |
| [React](https://react.dev/) | `19.2.7` | UI 组件模型 |
| [TypeScript](https://www.typescriptlang.org/) | `5.x` | 类型系统 |
| [Tailwind CSS](https://tailwindcss.com/) | `4.3.3` | 样式系统 |
| [Zustand](https://github.com/pmndrs/zustand) | `5.0.14` | 轻量状态管理 |
| [hls.js](https://github.com/video-dev/hls.js/) | `1.6.16` | HLS 播放 |
| [Lucide React](https://lucide.dev/) | `1.25.0` | 图标库 |
| [@dnd-kit](https://dndkit.com/) | `6.x / 10.x` | 拖拽排序 |
| [@upstash/redis](https://github.com/upstash/redis-js) | `1.38.0` | Cloudflare / 托管环境 Redis REST 同步与托管账户 |
| TCP Redis (`redis://` / `rediss://`) | - | Docker / 自托管 Node.js 托管账户、同步与登录限流 |
| [opencc-js](https://github.com/nk2028/opencc-js) | `1.4.1` | 繁简转换辅助搜索 |
...
| Playwright | `1.61.1` |
| OpenNext Cloudflare Adapter | `1.20.1` |
| next-on-pages | `1.13.16`（仅保留兼容用途） |
| TypeScript Runtime (`tsx`) | `4.23.1` |
...
- 项目主页：[bruceblink/KVideo](https://github.com/bruceblink/KVideo)
- 问题反馈：[GitHub Issues](https://github.com/bruceblink/KVideo/issues)

## Star History

[![Star History Chart](https://api.star-history.com/svg?repos=bruceblink/KVideo&type=date&legend=top-left)](https://www.star-history.com/#bruceblink/KVideo&type=date&legend=top-left)
