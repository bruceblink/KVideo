  ...
  return (
    <>
      ...
      <button
        ...
        style={floatingStyle}
        className="fixed z-40 bg-(--glass-bg) backdrop-blur-sm saturate-120 border border-(--glass-border) rounded-2xl shadow-(--shadow-md) p-3 hover:scale-105 transition-transform duration-200 cursor-pointer touch-none select-none"
        aria-label="打开观看历史"
        title="点击打开观看历史，拖动可调整位置"
      >
        <Icons.History size={24} className="text-(--text-color)" />
      </button>
      ...
      <aside
        ...
        className={`fixed top-0 right-0 bottom-0 w-[85%] sm:w-[90%] max-w-105 z-2000 bg-(--glass-bg) backdrop-blur-sm saturate-120 border-l border-(--glass-border) rounded-tl-2xl rounded-bl-2xl p-6 flex flex-col shadow-[0_8px_32px_rgba(0,0,0,0.2)] transition-transform duration-250 ease-out`}
      >
